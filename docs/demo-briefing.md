...
3. [The AI Model — Why Phi-3 Mini](#the-ai-model--why-phi-3-mini)
...
| GPU Node | 1 | NVIDIA A2 (16 GB VRAM) — Phi-3 inference |
...
└── AKS Arc Cluster
    ...
    └── GPU Node (NVIDIA A2)  ← Phi-3 AI inference
```
...
## The AI Model — Why Phi-3 Mini

**Talk track (Minute 5–6)**

### Model Overview

**Phi-3 Mini 4K Instruct** — Microsoft's compact 3.8 billion parameter small language model (SLM), purpose-built for constrained edge environments.
...
| **Phi-3 Summary** | SLM inference on GPU | 2–5 s | Depends on GPU availability |
...
2. **Phi-3 overlays asynchronously** — a one-sentence natural-language fleet health summary on top of the structured insights
3. **If the GPU is busy or unavailable**, the rule engine still provides useful insights — **the UI never goes blank**

### What the Model Sees

The prompt is **compact text** (not raw JSON) to keep the 3.8B model focused on analysis rather than echoing data:
...
1. **Downloads** Phi-3-mini-4k-instruct-cuda-gpu from the Foundry catalog (~2 GB)
2. **Schedules** the inference pod on the GPU node (NVIDIA A2)
3. **Creates** a ClusterIP service at `phi-3-deployment.foundry-local.svc:5000`
4. **Issues** TLS certificates via cert-manager
5. **Generates** an API key stored in a Kubernetes secret (`phi-3-deployment-api-keys`)
...
**What to point out:** GPU utilization spiking every 15 seconds — those are the AI inference bursts from Phi-3 analyzing the drone fleet. A modest edge GPU running real AI at ~26W power draw.
...
| **4–6 min** | AI insights panel + Grafana GPU | *"Phi-3 Mini on the NVIDIA A2 GPU analyzes the fleet every 15 seconds. Watch the GPU utilization spike."* |
