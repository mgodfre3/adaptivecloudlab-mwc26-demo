"""GPU metrics collector for edge AI dashboards.

Queries DCGM exporter (Prometheus text format) on the cluster to surface
real-time GPU utilisation, memory, temperature, and power draw.  Falls back
gracefully when the exporter is unreachable.
"""
...
# DCGM exporter endpoint — set via env or default to in-cluster service
...
METRIC_KEYS = {
    ...
    "DCGM_FI_PROF_GR_ENGINE_ACTIVE": "engine_active",
}
...
def fetch_gpu_metrics() -> dict:
    ...
    Returns a dict with keys like gpu_utilization, memory_used_mb, etc.
    Returns a fallback dict with status='unavailable' on failure.
    """
...
def _parse_dcgm(raw_text: str) -> dict:
    """Parse Prometheus text format and extract DCGM metrics."""
    result: dict = {}
